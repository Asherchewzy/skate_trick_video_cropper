...
def test_flow():
    if not os.path.exists(VIDEO_FILE):
        print(f"Error: {VIDEO_FILE} not found. Run create_dummy_video.py first.")
        sys.exit(1)
    ...
    if response.status_code == 200:
        with open("result_highlight.mp4", "wb") as f:
            f.write(response.content)
        print("Download successful: result_highlight.mp4")
    else:
